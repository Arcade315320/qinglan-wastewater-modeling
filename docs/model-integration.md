...
| 模型 | 用途 | 当前状态 |
| --- | --- | --- |
| ASM1 | 有机物去除、硝化、反硝化 | 可用于快速筛选计算 |
| ASM2d | 有机物、氮、强化生物除磷及化学除磷 | 可用于快速筛选计算，默认模型 |
...
当前快速计算层采用 QSDsan ASM2d 的典型动力学参数和 Monod/SRT/HRT 关系，
用于录入检查、工艺比较和参数敏感性筛选。它不是完整 Gujer 矩阵动态积分，
结果会在响应中的 `engine`、`model_note`、`assumptions` 和 `warnings` 明示。
...
1. 增加 COD、TN、TP 的组分化输入，不再只依赖默认比例。
2. 按实际池容建立厌氧、缺氧、好氧反应器和内外回流。
3. 接入二沉池或膜分离单元，并用实测 MLSS/SVI 校准固液分离。
4. 用至少一个月的进出水时序数据校准动力学参数。
5. 用独立时段验证，并报告 RMSE、MAPE、偏差和参数不确定性。
